...
rem ※このファイルは CRLF 改行・CP932 で保存すること。LF のみだと cmd.exe が
rem   行継続や一部の構文を正しく解釈できない（実障害 2026-08-28: URL が捨てられ
rem   Chrome が初期画面のまま開いた）。行継続の ^ も使わない。
...
:nochrome
echo Google Chrome が見つかりませんでした。インストール先を確認してください。
pause
